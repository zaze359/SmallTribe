...
import com.zaze.router.RouteCenter;
...
public class ClassUtils {
    ...
    private static final String DEX_DEPENDENCIES = "split_lib_dependencies_apk";
...
//     * @return Set<String> 所有class的集合
//     */
//    public static Set<String> getFileNameByPackageName(Context context, final String packageName) {
//        final Set<String> classNames = new HashSet<>();
//        try {
//            StartupStore startupStore = StartupStore.build();
//            loadInto(context, packageName, classNames, startupStore);
...
//    }
     ...
     * @param startupStore startupStore
     * @throws PackageManager.NameNotFoundException e
     * @throws IOException                          e
     * @throws InterruptedException                 e
     */
    public static void loadInto(final Context context, final String packageName, final Set<String> classNames, StartupStore startupStore) throws PackageManager.NameNotFoundException, IOException, InterruptedException {
        List<String> paths = getSourcePaths(context);
        for (final String path : paths) {
            startupStore.push(new StartupTask(path) {
                @Override
                public void doTask() {
                    DexFile dexfile = null;
                    try {
//                        if (path.contains(DEX_DEPENDENCIES)) {
//                            // dependencies 分包不需要扫描检查
//                            return;
//                        } else
                        ...
                        while (dexEntries.hasMoreElements()) {
                            ...
                            if (className.startsWith(packageName)) {
                                if (RouteCenter.loadClass(context, packageName, className)) {
                                    classNames.add(className);
                                }
                            }
                        }
                    } catch (Throwable ignore) {
                        Log.e("ClassUtils", "Scan map file in dex files made error.", ignore);
                    } finally {
                        if (null != dexfile) {
                            try {
                                dexfile.close();
...
//        通过是否存在sp中的multidex.version是不准确的，因为从低版本升级上来的用户，是包含这个sp配置的
        ...
        if (ZRouter.debuggable()) { // Search instant run support only debuggable
